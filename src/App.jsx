...
const [driverPassword, setDriverPassword] = useState('');
  ...
  // 🔄 دالة التوزيع التلقائي والمباشر
  const handleAutoDistribute = async (e, isAutomatic = false) => {
    if (e && e.preventDefault) e.preventDefault();
    const autoMode = typeof e === 'boolean' ? e : isAutomatic;

    if (!autoMode) {
      if (!window.confirm('هل أنت متأكد من إعادة توزيع الطلاب المداومين والاستثناءات على السائقين؟')) return;
    }
    ...
    try {
      ...
      if (dErr || sErr || !driversList || driversList.length === 0) {
        if (!autoMode) alert('⚠️ لا يوجد سائقون مسجلون أو حدث خطأ في جلب البيانات!');
        return;
      }

      // 1️⃣ معرفة اسم يوم غد بتوقيت بغداد
      ...
      // 2️⃣ تصفية الطلاب المداومين والاستثناءات
      const eligibleStudents = studentsData.filter((student) => {
        ...
        const hasExamNote = student.exam_note && String(student.exam_note).trim() !== '' && !String(student.exam_note).includes('لا أداوم غداً');
        const isExamException = (!isTomorrowInWorkDays && hasExamNote) || student.has_exception === true;

        return isTomorrowInWorkDays || isExplicitAttending || isExamException;
      });

      if (eligibleStudents.length === 0) {
        if (!autoMode) alert(`⚠️ لا يوجد طلاب لديهم دوام أو استثناءات ليوم غد (${tomorrowDay})!`);
        return;
      }

      // 3️⃣ ربط وتوزيع الطلاب المقبولين على السائقين
      for (let i = 0; i < eligibleStudents.length; i++) {
        const assignedDriver = driversList[i % driversList.length];
      ...
      // 4️⃣ إلغاء الربط عن الطلاب غير المداومين
      ...
      if (refreshed && typeof setStudents === 'function') setStudents(refreshed);

      if (!autoMode) {
        alert(`🎉 تم التوزيع بنجاح! توزّع (${eligibleStudents.length}) طالب على (${driversList.length}) سائق.`);
      }
    } catch (err) {
